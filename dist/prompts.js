...
export function buildSystemPrompt(args) {
    ...
    const enabledChecks = Object.entries(config.checks)
        ...
        .map(([k]) => k)
        .join(', ');
    const langInstruction = config.language === 'es'
...
${mergedRulesText || '(sin reglas — aplicá best practices generales)'}`,
    ...
    sections.push(`**Cómo señalar líneas:**
...
- Si el problema es sobre el archivo en general (no una línea específica), usá la primera línea cambiada del archivo y aclaralo en la descripción.`, `**Calidad sobre cantidad:**
